package com.example.myb.fisheyetopanoramavideo.player;
...
import com.example.myb.fisheyetopanoramavideo.gles.EGLRenderTarget;
import com.example.myb.fisheyetopanoramavideo.gles.GLHelpers;
import com.example.myb.fisheyetopanoramavideo.gles.SphericalSceneRenderer;

import java.io.IOException;

import static com.example.myb.fisheyetopanoramavideo.MainActivity.toast;
...
public class SphericalVideoPlayer extends TextureView {
    ...
    private float SPHERE_RADIUS = 500.f;
    ...
    private class RenderThread extends HandlerThread {
        ...
        private float[] rotationMatrix_x = new float[16];
        private float[] rotationMatrix_y = new float[16];
        private float[] viewMatrix = new float[16];
        private float[] projectionMatrix = new float[16];

        private float[] camera = new float[3];

        private float lon = 90; // longitude (0' - 360')
        private float lat = 0; // latitude (0' - 360')
        ...
        private void onSurfaceAvailable(SurfaceTexture surfaceTexture, int width, int height) {
            ...
            // x 轴方向 -----------
            //Matrix.setRotateM(rotationMatrix_x, 0, INITIAL_PITCH_DEGREES, 1, 0, 0);
            // y 轴方向, 向下, z 轴向屏幕里...
            //Matrix.setRotateM(rotationMatrix_y, 0, INITIAL_PITCH_DEGREES, 0, 1, 0);
            //Matrix.setIdentityM(rotationMatrix_y, 0);
            //Matrix.multiplyMM(modelMatrix, 0, rotationMatrix_x, 0, rotationMatrix_y, 0);
            // 不旋转
        ...
        private void updateCamera() {
            ...
            camera[0] = (float)(SPHERE_RADIUS * Math.sin(theta) * Math.cos(phi));
            camera[1] = (float)(SPHERE_RADIUS * Math.sin(theta) * Math.sin(phi));
            camera[2] = (float)(SPHERE_RADIUS * Math.cos(theta));

            //android.util.Log.e("mybb", "p,t,c1,c2,c3 = " + phi + " " + theta + " " + camera[0] + " " + camera[1] + " " + camera[2]);
            Matrix.setLookAtM(
                    viewMatrix, 0,
                    0, 0, 0,
        ...
        private void onScroll(ScrollDeltaHolder deltaHolder) {
            lon = (deltaHolder.deltaY) * DRAG_FRICTION + lon;
            lat = (deltaHolder.deltaX) * DRAG_FRICTION + lat;
